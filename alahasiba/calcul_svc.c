/*
 * Please do not edit this file.
 * It was generated using rpcgen.
 */

#include "calcul.h"
#include <stdio.h>
#include <stdlib.h>
#include <rpc/pmap_clnt.h>
#include <string.h>
#include <memory.h>
#include <sys/socket.h>
#include <netinet/in.h>

#ifndef SIG_PF
#define SIG_PF void(*)(int)
#endif

static int *
_add_1 (struct operands  *argp, struct svc_req *rqstp)
{
	return (add_1_svc(*argp, rqstp));
}

static int *
_subtract_1 (struct operands  *argp, struct svc_req *rqstp)
{
	return (subtract_1_svc(*argp, rqstp));
}

static int *
_multiply_1 (struct operands  *argp, struct svc_req *rqstp)
{
	return (multiply_1_svc(*argp, rqstp));
}

static int *
_divide_1 (struct operands  *argp, struct svc_req *rqstp)
{
	return (divide_1_svc(*argp, rqstp));
}

static int *
_abs_value_1 (struct operands  *argp, struct svc_req *rqstp)
{
	return (abs_value_1_svc(*argp, rqstp));
}

static void
calculator_prog_1(struct svc_req *rqstp, register SVCXPRT *transp)
{
	union {
		struct operands add_1_arg;
		struct operands subtract_1_arg;
		struct operands multiply_1_arg;
		struct operands divide_1_arg;
		struct operands abs_value_1_arg;
	} argument;
	char *result;
	xdrproc_t _xdr_argument, _xdr_result;
	char *(*local)(char *, struct svc_req *);

	switch (rqstp->rq_proc) {
	case NULLPROC:
		(void) svc_sendreply (transp, (xdrproc_t) xdr_void, (char *)NULL);
		return;

	case ADD:
		_xdr_argument = (xdrproc_t) xdr_operands;
		_xdr_result = (xdrproc_t) xdr_int;
		local = (char *(*)(char *, struct svc_req *)) _add_1;
		break;

	case SUBTRACT:
		_xdr_argument = (xdrproc_t) xdr_operands;
		_xdr_result = (xdrproc_t) xdr_int;
		local = (char *(*)(char *, struct svc_req *)) _subtract_1;
		break;

	case MULTIPLY:
		_xdr_argument = (xdrproc_t) xdr_operands;
		_xdr_result = (xdrproc_t) xdr_int;
		local = (char *(*)(char *, struct svc_req *)) _multiply_1;
		break;

	case DIVIDE:
		_xdr_argument = (xdrproc_t) xdr_operands;
		_xdr_result = (xdrproc_t) xdr_int;
		local = (char *(*)(char *, struct svc_req *)) _divide_1;
		break;

	case ABS_VALUE:
		_xdr_argument = (xdrproc_t) xdr_operands;
		_xdr_result = (xdrproc_t) xdr_int;
		local = (char *(*)(char *, struct svc_req *)) _abs_value_1;
		break;

	default:
		svcerr_noproc (transp);
		return;
	}
	memset ((char *)&argument, 0, sizeof (argument));
	if (!svc_getargs (transp, (xdrproc_t) _xdr_argument, (caddr_t) &argument)) {
		svcerr_decode (transp);
		return;
	}
	result = (*local)((char *)&argument, rqstp);
	if (result != NULL && !svc_sendreply(transp, (xdrproc_t) _xdr_result, result)) {
		svcerr_systemerr (transp);
	}
	if (!svc_freeargs (transp, (xdrproc_t) _xdr_argument, (caddr_t) &argument)) {
		fprintf (stderr, "%s", "unable to free arguments");
		exit (1);
	}
	return;
}

int
main (int argc, char **argv)
{
	register SVCXPRT *transp;

	pmap_unset (CALCULATOR_PROG, CALCULATOR_VERS);

	transp = svcudp_create(RPC_ANYSOCK);
	if (transp == NULL) {
		fprintf (stderr, "%s", "cannot create udp service.");
		exit(1);
	}
	if (!svc_register(transp, CALCULATOR_PROG, CALCULATOR_VERS, calculator_prog_1, IPPROTO_UDP)) {
		fprintf (stderr, "%s", "unable to register (CALCULATOR_PROG, CALCULATOR_VERS, udp).");
		exit(1);
	}

	transp = svctcp_create(RPC_ANYSOCK, 0, 0);
	if (transp == NULL) {
		fprintf (stderr, "%s", "cannot create tcp service.");
		exit(1);
	}
	if (!svc_register(transp, CALCULATOR_PROG, CALCULATOR_VERS, calculator_prog_1, IPPROTO_TCP)) {
		fprintf (stderr, "%s", "unable to register (CALCULATOR_PROG, CALCULATOR_VERS, tcp).");
		exit(1);
	}

	svc_run ();
	fprintf (stderr, "%s", "svc_run returned");
	exit (1);
	/* NOTREACHED */
}
